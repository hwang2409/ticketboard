...
| `TICKETBOARD_CODEX_STDIN_TRANSPORT` | Optional `auto`, `direct`, or `pty` override for Codex brief stdin handling. Defaults to pseudo-terminal mode when the generator has no terminal stdin. |
| `TICKETBOARD_CODEX_TIMEOUT_MS` | Maximum time for one Codex brief generation. Defaults to 8 minutes. |
